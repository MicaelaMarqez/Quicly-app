  ...
  return (
    <NativeBaseProvider>
      <ScrollView style={styles.container}>
        <ImageBackground
          style={styles.image}
          source={{ uri: 'https://i.postimg.cc/DwvFPSkY/comida.gif' }}
        >
          <ImageBackground
            style={styles.logo}
            source={{ uri: 'https://i.postimg.cc/zv3pNmMr/loguito-2.png' }}
          ></ImageBackground>
          <View style={styles.cart}>
            <TextInput
              placeholder='¿Qué quieres pedir?'
              style={styles.searcher}
              onChange={(e) => getFiltered(e.nativeEvent.text)}
            />
        ...
        {filteredView ? (
          !props.filtered.length ? (
            ...
            props.filtered.map((product) => (
              <CategoryList product={product} key={product._id} route={props.route} />
            ))
          )
        ) : (
          categories
